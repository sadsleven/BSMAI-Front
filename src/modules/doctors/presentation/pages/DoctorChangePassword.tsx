...
/** Cambio de contraseña (admin) del acceso de un doctor proveedor. */
export function DoctorChangePassword() {
  ...
  const onSubmit = async (values: Values) => {
    if (!id) return;
    try {
      await doctorGateway.changePassword(id, {
    ...
    <div className="max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit, (errs) => notifyFormErrors(errs))} className="space-y-6" noValidate>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          >
            <ChevronLeft className="w-3.5 h-3.5" /> Volver al doctor
          </button>
        </div>

        <FormSection
          title="Contraseña"
          description="Definí la contraseña de acceso del doctor como usuario proveedor."
        >
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          <Button type="button" variant="outline" onClick={() => navigate(`/doctors/edit/${id}`)}>
            Cancelar
          </Button>
