...
export function PathologyEdit() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const p = await pathologyGateway.getById(id);
        reset({
          name: p.name,
        ...
        notify.fromError(e, 'No se pudo cargar la patología.');
      } finally {
        setFetching(false);
      }
    })();
  ...
  const onSubmit = async (values: PathologyValues) => {
    if (!id) return;
    try {
      await pathologyGateway.update(id, {
        name: values.name,
        description: values.description ?? null,
        isActive: values.isActive,
      });
      notify.success('Patología actualizada');
      navigate('/pathologies');
    } catch (err) {
      notify.fromError(err, 'No se pudo actualizar la patología.');
    }
  };
