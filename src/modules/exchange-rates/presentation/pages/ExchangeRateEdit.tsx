...
export function ExchangeRateEdit() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await exchangeRateGateway.getById(id);
        reset({
          currency: r.currency,
          amountBs: Number(r.amountBs),
          effectiveDate: r.effectiveDate,
          isActive: r.isActive ?? true,
        });
        setDisplayLabel(
          `${r.currency} — ${new Date(r.effectiveDate).toLocaleString('es-VE')}`,
        );
      } catch (e) {
        notify.fromError(e, 'No se pudo cargar la tasa de cambio.');
      } finally {
        setFetching(false);
      }
    })();
  ...
  const onSubmit = async (values: ExchangeRateValues) => {
    if (!id) return;
    try {
      await exchangeRateGateway.update(id, {
      ...
      notify.fromError(err, 'No se pudo actualizar la tasa de cambio.');
    }
  };
