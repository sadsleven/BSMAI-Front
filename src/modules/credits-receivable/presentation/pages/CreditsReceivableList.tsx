...
import { CreditsReceivableDetail } from '../components/CreditsReceivableDetail';
import {
  Table,
  TableBody,
  TableCell,
...
export function CreditsReceivableList() {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      ...
      setError(getHttpErrorMessage(e, 'No se pudieron cargar los créditos'));
    } finally {
      setLoading(false);
    }
  }, [filters]);
  ...
  const goRegister = () => {
    if (selectedAccounts.length === 0) {
      notify.warning('Seleccioná al menos un crédito');
      return;
    }
    navigate('/credits-receivable/register-collection', {
      state: { creditIds: selectedAccounts.map((a) => a.id) },
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {metadata.total.toLocaleString()} crédito{metadata.total === 1 ? '' : 's'} en total
          </p>
        </div>
        <Can permission={PERMISSIONS.CREDITS_RECEIVABLE.UPDATE}>
      ...
      <div className="bg-card rounded-xl border shadow-xs overflow-hidden">
        <DataTableToolbar
          ...
          searchPlaceholder="Buscar por N° crédito, N° orden o titular…"
          ...
          filters={
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="partially_collected">Cobrado parcialmente</SelectItem>
                  ...
                  <EmptyState
                    ...
                    title={
                      hasActiveFilters
                        ...
                        : 'Sin créditos por cobrar'
                    }
                    description={
                      hasActiveFilters
                        ? 'Limpiá los filtros para ver todos los créditos.'
                        : 'Los créditos se generan al crear órdenes tipo crédito.'
                    }
                  />
                </TableCell>
              </TableRow>
