...
- Reglas duras: nombre 1-150 chars solo letras/acentos/`ñ`; teléfono opcional pero exactamente 11 dígitos si presente; password 8-100 con mayúscula+minúscula+número+especial; cédula `V/E-XX.XXX.XXX`; RIF `J/G/V/E-XXXXXXXX-D` (sin puntos); teléfonos para owners 11 dígitos exactos.
...
- `<RifInput>` (`src/components/ui/rif-input.tsx`) — auto-formato `J-XXXXXXXX-D` (RIF sin puntos). Acepta `J/G/V/E`. Misma regla de uso obligatorio.
